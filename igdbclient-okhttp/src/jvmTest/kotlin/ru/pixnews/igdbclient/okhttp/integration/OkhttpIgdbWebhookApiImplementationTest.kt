...
import ru.pixnews.igdbclient.okhttp.OkhttpIgdbConstants
import ru.pixnews.igdbclient.okhttp.integration.MockWebServerExt.setupTestOkHttpClientBuilder

internal class OkhttpIgdbWebhookApiImplementationTest {
    @JvmField
    @RegisterExtension
    ...
    companion object {
        private const val SINGLE_WEBHOOK_RESPONSE: String = """
[
  {
    "id": 7136,
    ...
    "updated_at": 1688017855
        ...
        private fun createSuccessMockResponse(response: String = SINGLE_WEBHOOK_RESPONSE) = MockResponse()
            .setResponseCode(200)
            .setHeader("Content-Type", OkhttpIgdbConstants.MediaType.APPLICATION_JSON)
            .setBody(response)
    }
    ...
    inner class RegisterWebhookTests {
        ...
        fun `registerWebhook() should send correct request`() = coroutinesExt.runTest {
            ...
            server.takeRequest().run {
                ...
                headers.values("Content-Type") shouldBe listOf("application/x-www-form-urlencoded")
            }
        }
    }
